...
# import starting player data into a new file
    ...
    file = open("smiles_DataFiles.py", "w+")
    stat_block = f"""
# define player stats
player_level = 1
player_health = 48
 ...
 ______________    ______________    ______________ """)
            ...
            if distance == 2:
                if enemy_health > 8:
                    ...
                    if random.randint(1, 4) == 1:
                        sprint(
                            'The creature, close to death, flees off into the forest.'
                        )
            ...
            if distance < 3:
                if distance == 1:
                    attack_chance = (random.randint(1, 20) +
                                     creature_attackbonus)
                else:
                    attack_chance = (random.randint(1, 20))
                if attack_chance > player_ac:
                    enemy_attack = (random.randint(1, 8) +
                                    creature_attackbonus)
                    player_health -= enemy_attack
                    if player_health < 0:
                        player_health = 0
                    sprint(
                        f'The creature attacked dealing {enemy_attack} damage. You are at {player_health} hp.'
                    )
                    if player_health == 0:
                        sprint('You died.')
                else:
        ...
        def reward():
            ...
            if enemy_health == 0:
                ...
                sprint(
                    f'You got {gold} gold, {reward_arrows} arrows, {reward_potions} health potions, and {experience} experience.'
                )
                mana = mana + 20
                ...
                if total_experience >= (3000 * player_level):
                    ...
                    player_ac = (4 + player_level)
                    sprint(
                        f'You leveled up. You are now at level {player_level}.'
                    )
        ...
        # opening alert
        sprint(
            f'You step out onto the dirt trail outside the tavern and journey off deep into the forest ahead.'
        )
        sprint(
            f'A short walk later, a {enemy_size} creature has approached you. It appears hostile.'
        )
        sprint("""------------------------
1 attack with sword
...
------------------------""")
                ...
                if distance == 1:
                    ...
                    if attack_chance > creature_ac:
                        ...
                        if enemy_health < 0:
                            enemy_health = 0
                        sprint(
                            f'You swung you sword at the creature dealing {player_attack} damage. The creature is at {enemy_health} hp.'
                        )
                        if enemy_health == 0:
            ...
            if player_action == "2":
                if total_arrows > 0:
                    ...
                    if distance == 1:
                        attack_chance = (random.randint(1, 20) -
                                         (6 + player_attackbonus))

                    else:
                        attack_chance = (random.randint(1, 20) +
                                         player_attackbonus)
                    if attack_chance > creature_ac:
                        player_attack = (random.randint(1, 12) +
                                         player_attackbonus)
                        enemy_health -= player_attack
                        if enemy_health < 0:
                            enemy_health = 0
                        sprint(
                            f'You shot the creature with your bow doing {player_attack} damage. The creature is at {enemy_health} hp.'
                        )
                        if enemy_health == 0:
                            sprint('The creature died.')
                            reward()
...
------------------------""")
                    ...
                    if mana >= 10:
                        ...
                        if attack_chance > creature_ac:
                            ...
                            if enemy_health < 0:
                                enemy_health = 0
                            sprint(
                                f'You cast flame dealing {player_attack} damage. The creature is at {enemy_health} hp.'
                            )
                            if enemy_health == 0:
                                sprint('The creature died.')
                                reward()
                    ...
                    if mana >= 30:
                        ...
                        if attack_chance > creature_ac:
                            ...
                            if enemy_health < 0:
                                enemy_health = 0
                            sprint(
                                f'You cast lightening bolt dealing {player_attack} damage. The creature is at {enemy_health} hp.'
                            )
                            sprint(
                                'The creature is temporarily paralyzed. It does not attack.'
                            )
                            if enemy_health == 0:
                                sprint('The creature died.')
                                rewrad()
                ...
                if spell_cast == '3':
                    if mana >= 20:
                        ...
                        player_health += 10
                        sprint(
                            f'You cast heal. You recovered 10 hp. You are at {player_health} hp.'
                        )
                    else:
                        sprint('You do not have enough mana.')
                        continue
                ...
                while 1 > 0:
                    ...
                    if inventory_action == '1':
                        if total_health_potions > 0:
                            ...
                            sprint(
                                f'You took a potion of health. You recovered {health_from_potion} hp. You are at {player_health} hp. You have {total_health_potions} potions left.'
                            )
                            continue
                        else:
                    ...
                    if inventory_action == '2':
                        if total_mana_potions > 0:
                            ...
                            sprint(
                                f'You took a potion of mana. You recovered 20 mp. You are at {mana} mp. You have {total_mana_potions} potions left.'
                            )
                            continue
                        else:
...
------------------------""")
                ...
                if move_action == '2':
                    if distance == 1:
                        ...
                        move_action = input(
                            'You are already stepped away from the creature. Do you want to run away? (y or n)'
                        )
                        if move_action == 'y':
                            sprint('You ran away.')
                            rewrad()
    ...
    if tavern_action == '2':
        if drink <= 2:
            sprint(
                '\"Hunter, I see the monsters haven\'t gotten you yet. Can\'t say the same for all your peers.\"'
            )
            player_action = input('\"I take it you want a drink?\" (y or n)')
            if player_action == 'n':
                sprint('\"To early for some I guess\"')
                player_action = input(
                    '\"I take it you\'re here for the special bounty than?\" (y or n)'
                )
                if player_action == 'y':
                    sprint(
                        '\"Ah, just head over the bounty board and choose the "special" one, but i warn you, its a tough one... be safe.\"'
                    )
                    continue
                else:
            ...
            if player_action == 'y':
                sprint(
                    '\"Ah, one to have some fun in battle I see. I knew I liked you\"'
                )
                if total_gold >= 5:
                    total_gold -= 5
            ...
            sprint(
                '\"Careful, you gotta slow down before I gotta take that sword from you.\"'
            )
    ...
    if tavern_action == '3':
        sprint(
            '\"So you\'re looking for some provisions? I have some stuff under the bar.\"'
        )
        print(f"""------------------------------
...
------------------------------""")
    ...
    if tavern_action == ('0'):
        ...
        # save player data to existing file 
        file = open("smiles_DataFiles.py", "w+")
        stat_block = f"""
# define player stats
...
total_health_potions = {total_health_potions}
total_mana_potions = {total_mana_potions}
